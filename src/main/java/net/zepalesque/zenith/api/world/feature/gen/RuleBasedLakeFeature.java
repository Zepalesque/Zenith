...
public class RuleBasedLakeFeature extends Feature<RuleBasedLakeFeature.Config> {
    ...
    public RuleBasedLakeFeature(Codec<Config> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<Config> context) {
        ...
        if (blockpos.getY() <= worldgenlevel.getMinBuildHeight() + 4) return false;
                ...
                for (int xTest = 1; xTest < 15; ++xTest)
                    for (int zTest = 1; zTest < 15; ++zTest)
                        for (int yTest = 1; yTest < 7; ++yTest) {
                            ...
                            if (sizeSquare < 1.0) shouldPlace[packPos(xTest, yTest, zTest)] = true;
                        }
            }
            ...
            for (x = 0; x < 16; ++x)
                for (z = 0; z < 16; ++z)
                    for (y = 0; y < 8; ++y) {
                        boolean flag = shouldPlace(shouldPlace, x, z, y);
                        if (flag) {
                            BlockState testState = worldgenlevel.getBlockState(blockpos.offset(x, y, z));
                            if (y >= 4 && testState.liquid()) return false;
                ...
                for (x = 0; x < 16; ++x)
                    for (z = 0; z < 16; ++z)
                        for (int y1 = 0; y1 < 8; ++y1) {
                            ...
                            boolean flag2 = shouldPlace(shouldPlace, x, z, y1);
                            if (flag2 && (y1 < 4 || random.nextInt(2) != 0)) {
                                ...
                                if (blockstate.isSolid() && !blockstate.is(BlockTags.LAVA_POOL_STONE_CANNOT_REPLACE)) {
                                    ...
                                    if (!blockstate2.isAir()) worldgenlevel.setBlock(blockpos3, blockstate2, 2);
                                }
                            }
                        }
    ...
    private boolean shouldPlace(boolean[] positions, int x, int z, int y) {
        return !positions[packPos(x, y, z)]
            && (x < 15 && positions[packPos(x + 1, y, z)]
            || x > 0 && positions[packPos(x - 1, y, z)]
            || z < 15 && positions[packPos(x, y, z + 1)]
            || z > 0 && positions[packPos(x, y, z - 1)]
            || y < 7 && positions[packPos(x, y + 1, z)]
            || y > 0 && positions[packPos(x, y - 1, z)]);
    }
