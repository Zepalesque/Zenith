...
public class MathUtil {
    ...
    public static double clampedInverseLerp(double start, double end, double delta) {
        if (delta < start) return 0.0;
        else return delta > end ? 1.0 : Mth.inverseLerp(delta, start, end);
    }

    public static float clampedInverseLerp(float start, float end, float delta) {
        if (delta < start) return 0.0F;
        else return delta > end ? 1.0F : Mth.inverseLerp(delta, start, end);
    }
